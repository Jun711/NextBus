...
public class TranslinkService extends AbstractTranslinkService {
	 ...
	 */
	private final static String APIKEY = "7AyOQyqtv59NnvhSSXKD";  // insert your Translink key here
    ...
    public void addWaitTimeEstimatesToStop(BusStop stop) throws TranslinkException {
		...
		StringBuilder uriBuilder = new StringBuilder("//api.translink.ca/RTTIAPI/V1/");
		uriBuilder.append("stops/" + Integer.toString(stop.getStopNum()).trim() + "/estimates?");
		//uriBuilder.append("apikey=" + APIKEY);
		uriBuilder.append("count=3&timeframe=60&");
		uriBuilder.append("apikey=" + APIKEY);
		//uriBuilder.trimToSize();
		
		//System.out.println(Integer.toString(stop.getStopNum()).trim());
    ...
    public void addBusLocationsForStop(BusStop stop)
		...
		uriBuilder.append("&stopNo=" + Integer.toString(stop.getStopNum()).trim());
